...
| ☐ | 代码 | **plan 门禁消费方**:后端 `require_plan()` 装饰逻辑 + 前端按 `plan` 显隐 | free 账号打 Pro-only 能力→402/403;Pro 放行 | P0-3 |
...
| ☐ | 代码 | **遥测同意/opt-out 开关**:设置页开关 + 前端尊重 + 兑现 opt-in 承诺 | 关闭后不发 telemetry/feedback;默认状态与隐私页一致 | P1 遥测同意 |
