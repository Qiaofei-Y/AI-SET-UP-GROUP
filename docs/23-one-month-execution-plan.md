...
| ☑ | 代码 | **密码找回**:`/v1/auth/forgot` + `/v1/auth/reset` | ✅ forgot 恒定 `200 {ok:true}`(防枚举),仅真实账号才发含一次性令牌的重置信;reset 兑付令牌(只存 `sha256`,单次使用,1h 时效)→ 换哈希 + **撤销全部 session** + 顺带 `email_verified=1`。测试:no_enumeration / flow_and_revokes_sessions / bad_shapes / token_never_stored_cleartext。**复审补正(2026-09-01)**:恒定响应体不足以防枚举——真实账号才做的令牌铸写 + 同步 SMTP 往返是**计时侧信道**;生产(配 SMTP)下把整个副作用移出请求线程(守卫线程),命中/未命中两路均只做一次 SELECT,dev/测试仍 inline(OUTBOX 与令牌即时可读)。新增测试 `forgot_dispatches_email_off_request_thread` | P0-15 |
...
| ☑ | 代码 | **卖点对齐复查**:对新增支付/账号功能再查一遍无新假象 | ✅ 双面审查(营销页 + app/新功能页,含 build/chat/account/checkout JS 文案,中英双语):支付/账号/同意面均诚实(dormant 计费显示 503 不假装扣款、离线操作 fail-closed、遥测 opt-in、dashboard 全 sample/preview 标注)。修正 3 处自相矛盾——「One-click local install」(Free 档 + 首页第 4 步)与 deploy 页自评「一键=即将推出」冲突,改为「引导式安装(Ollama)」;并给自动续费披露加 Beta 前置说明(不再在免费期声称「已在扣款」)。无「未实现却宣称可用」残留;security 167 + smoke 23 全绿。**复审补正(2026-09-01)**:三线对抗式复审又揪出两处「coming-soon 当已上线」回潮——① 首页 `og:title`/`twitter:title`/`<title>` 新引入「in one click / 一键」(正文早已按本条清掉,却漏进新加的社交 meta),改为「all in one place / 一站搞定」与 hero 对齐;② dashboard 把「Advanced RAG + citations」当 `● live` 当前解锁功能且无限定词(build 向导/安装器均标 RAG「Planned — ships with the desktop app」,pricing 标「(live in the demo)」),补「(demo)」限定词对齐 | P0-6 |
...
| ☑ | 代码 | **CI/CD**:GitHub Actions 跑 `frontend/tests/run.sh` + `backend/tests/api.test.py` | ✅ `.github/workflows/tests.yml`:backend / frontend 两个并行 job,PR + push-to-main 触发;backend 起真实服务跑真实 HTTP,frontend 在 ubuntu-latest(自带 Chrome)跑完整套(静态 + 单元 + XSS + 全页 smoke)。**✅ 分支保护已开(2026-09-01)**:免费私有仓库不支持分支保护/Rulesets,遂将仓库改为**公开**(密钥扫描先行:工作树 + 全历史均无提交的密钥/凭据)后,建 `main-protection` ruleset(active)——必需两个 check(`Backend API tests` + `Frontend security + XSS + UI smoke`)、要求走 PR(0 审批不锁属主)、禁删除/禁 force-push;红则挡合并已真正生效 | P1 CI/CD |
...
> **接续**:批次 2 · 付费交付物本体的可勾选执行清单见 [docs/24 批次 2 执行计划](24-batch-2-execution-plan.md)(2026-09-01 起)。
