...
| 3 | 购买→解锁门禁闭环 | 前后端 | M | **✅ 授权侧已修(2026-08-27)**:plan 仍服务端权威——webhook 经 `apply_subscription()`(按 customer id)是付费账号唯一变更路径,`set_plan()` 降为运营 CLI;注册一律落 free,自报值仅记 `plan_intent`。剩:「功能因 plan 开关」的门禁消费方(后端 require_plan + 前端按 plan 显隐,第 1 周) |
