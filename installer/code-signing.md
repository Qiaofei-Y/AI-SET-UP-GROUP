# 代码签名:Azure Trusted Signing vs 传统 EV token(P0-8)
...
| **成本模型** | 按月订阅(basic tier 低) + 按签名量;新公司门槛在**主体验证**而非钱 | 证书年费(数百刀级)+ token/HSM 费 |
...
1. **本项目要在 CI 自动出签名安装器**(docs/24 P0-8 目标),ATS 是唯一能在 GitHub 托管 runner 上**全自动、无密钥**签名的路径。
2. 免自管 HSM/token,减少一个「人保管密钥」的运维与丢失风险面。
3. 与已有基建同源心智(Stripe/SES 等美国托管服务),运维一致。
...
- **激活前两件事**:① 把 action 版本 pin 到具体 commit SHA;② desktop 构建 job 产出 `dist/*.msi`(需 Rust/Tauri,见 docs/24 本体)。
