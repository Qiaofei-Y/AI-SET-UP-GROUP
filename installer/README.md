# installer/ — 批次 2 安装器契约(代码地基)

> 这是**批次 2 安装器**的**决策无关代码地基**:在真正的 Tauri 安装器落地前,先把「向导产出 ↔ 安装器读取」的契约与运行时决策固化成**单一真相源 + 可测断言**,让后续开发不会漂移、也不会重复踩坑。
>
> **诚实边界(硬约束)**:本目录的内容**尚未上线**,不面向用户。今天用户从向导拿到的仍是 **Ollama 引导安装包**(真实可用,见 `frontend/assets/build.js` 与 `downloads.html` 的「即将推出」诚实标注)。此处描述的 llama.cpp 自包含安装器是批次 2 的**目标**,签名安装器就绪前 `downloads.html` 不放真实下载按钮。
...
- **待外部动作**:首个签名构建时测得真实 digest 写回 `runtime.json`;代码签名证书(长周期)。
