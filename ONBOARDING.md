...
python3 backend/tests/api.test.py        # 后端:39 项,起真实服务打真实 HTTP
```
...
- **已完成**:演示站全站(13 页,双语 + FX 动效层,含法律三件套草案 + 注册 clickwrap)、后端六组端点 v0(advise/registry/license/telemetry/feedback/auth)、前后端打通(部署同源拓扑)、生产化地基(限速/fail-closed 密钥/`--host`)、131+39 项测试体系。
